...
We will use a data base that contain stream fish species from Paraná and Paraguai basins. These comprises two of the main basins in Neotropical region. This data are embbeded in the package and can be easily accessed by typing:
...
taxon_data <- FishTaxaMaker(data = data_comm)
Loricariidae
Siluriformes
```
...
phylo_fish_streams <- FishPhyloMaker(data = taxon_data, return.insertions = T)
Hisonotus 
Synbranchidae 
```

In this example I choose to insert the species *Curculionichthys insperatus* near to Genus *Hisonotus*, and species *Synbranchus marmoratus* at the 
    base of its family, that is Synbranchidae.
...
With this code we will obtain a phylogenetic tree like that in Figure 1. The species that are not shaded with colours corresponds to monotipic families.

![Fig. 1 - Phylogenetic tree containing 59 species of stream fishes in the Paraná and Paraguay Basins (Brasil).](../vignettes/Phylo_Parana-Paraguai.png)
